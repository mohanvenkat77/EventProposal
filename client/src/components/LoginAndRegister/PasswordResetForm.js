...
import { loginToAccount, passwordReset, secretCheck } from "../../utills/api-utill";
...
export default function PasswordResetForm({ setIsreset }) {
    ...
    function onFormSubmit(e) {
        ...
        setBoo(false);
        secretCheck(loginUser, option ? "vendor" : "user")
            .then(res => {
    ...
    function onPasswordResetSubmit(e) {
        ...
        setBoo(false);
        passwordReset(loginUser, option ? "vendor" : "user")
            .then(res => {
                if (res.status === "Success") {
                    setLoginUser({
                        email: "",
            ...
            <div className="signin-form-container">
                ...
                <form onSubmit={onFormSubmit}>
                    ...
                    <div className="field-container">
                        <input type="text" id="secret" placeholder="Secret word for password reset" style={error.secret ? { border: "1px solid red" } : {}} required onChange={e => {
                    ...
                    <div className="btn-link-container margin">
                        <p className="createLink" onClick={() => setIsreset(true)}>Sign In</p>
                        <button type="submit">{boo ? "NEXT" : <span className="loader"></span>}</button>
                    </div>
                </form>
            </div> :
            <div className="signin-form-container">
                <form onSubmit={onPasswordResetSubmit}>
                    ...
                    <div className="field-container margin">
                        <input type="password" id="password" placeholder="New password" minLength={8} style={error.password ? { border: "1px solid red" } : {}} required onChange={e => {
                    ...
                    <div className="field-container margin">
                        <input type="password" id="confirmpassword" placeholder="Confirm password" minLength={8} style={error.password ? { border: "1px solid red" } : {}} required onChange={e => {
                            setLoginUser(ex => ({ ...ex, confirmPassword: e.target.value }));
                            setError(ex => ({ ...ex, password: "" }));
                        }} />
                    </div>
                    <div className="btn-link-container margin">
                        <p className="createLink" onClick={() => setIsreset(false)}>Sign In</p>
                        <button type="submit">{boo ? "SAVE" : <span className="loader"></span>}</button>
                    </div>
                </form>
            </div>
        }
